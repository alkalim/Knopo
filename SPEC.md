...
- **The caret stays on screen.** Any move that could take it past the edge — arrowing between blocks, `Enter`, typing that wraps a new line — scrolls the outline just far enough to bring it back, with a line of margin below it. (Each block is its own one-block-tall editor, so the text system's own caret scrolling has nothing to scroll.)
...
- Clicking a bullet **zooms** into that block (it becomes the temporary page root, with a breadcrumb back). Clicking the fold triangle toggles `collapsed`. An empty leaf block hides its bullet while unfocused (the gutter is kept, so nothing shifts) — except when it is the page's only block, where hiding it would leave the page showing nothing at all.
- **Opening a page can put the caret where you'd write.** Today's journal is a writing surface, so presenting it focuses an empty block at the end — appending one when the day already has content, and reusing the one that's already there otherwise (so revisiting never stacks blank blocks). Past journal days and ordinary pages don't: they open quiet, except when there is nothing to read at all — a single empty block, which is what a page you have only linked to looks like. Appending is an in-memory change with no save scheduled, so *visiting* the journal can never dirty its file; the block reaches disk when you type in it (§10). The block is also **scrolled into view** — on a day taller than the window the caret would otherwise be below the fold, making the whole thing look like it had done nothing.
- **A blank block draws a hint** while it is the outline's only row *and focused*: "Start typing, or / for commands" (§5.5). Only while focused — a hint in a block you are not editing reads as the page's content, the more so because an empty leaf hides its bullet. Unfocused, that block shows its bullet instead (§5.2), which says "a block lives here" without pretending to be text. It is drawn, not inserted — never part of the block's text, so never saved, indexed, or matched by find. A trailing block on a day that already has content gets a caret and no hint; so does an empty block created mid-outline. Right-sidebar panes never take focus this way — they are for reference. The converse does apply: a main outline opening ready to write takes focus *from* a pane that was being edited (the pane ends its own editing, committing what was typed), because a pane is a reference card while the main view is where you work. It never takes focus from another main outline — a journal-feed sibling, or the day the rollover just added while you were typing in another.
- **An outline with no blocks at all** — a page whose file has been emptied, a preamble-only file, or a zoom into a childless block — is given one, so every empty outline looks the same rather than having a second, click-to-create affordance of its own. As above the block is created in memory, so navigating never writes to the file.
...
- **`Cmd+J`** takes you to today — every press, never a toggle. It opens the journal home (staying put when the feed is already showing, so holding it down doesn't cycle between two views) and puts the caret in today's writing block. That request is explicit, so unlike the automatic "opens ready to write" it fires however often it is asked for and *may* take the caret from another day in the feed. Today's own page is reached by clicking its heading.
- **The day rolls over live.** An app left open past midnight adds the new day at the top of the journal home itself; the day that just ended moves down and stays there if it has content, like any past day. Clock and time-zone changes, and waking after sleeping through midnight, roll over the same way.
...
- **Left sidebar**: Journal (home), Favourites, Recents, Tags, All Pages. Shown and hidden with `Ctrl+Cmd+S` or the toolbar's collapse button. The Favourites, Recents, and Tags sections are collapsible (state persisted) and capped at 15 entries each in v1. All Pages shows unnamespaced regular pages in a collapsible Pages section, followed by collapsible namespace sections, then journal pages in a final collapsible Journal section with formatted date titles and newest days first. All Pages sections start expanded, save their collapse state per graph, and temporarily reveal matching rows while filtering without changing that saved state.
...
- **Search (`Cmd+K`)**: single dialog combining fuzzy page-name match (top section) and full-text block search (below), with `Enter` to navigate and `Cmd+Enter` to open in right sidebar. Full-text index lives in `cache.db`. Block search is FTS5 **token-prefix** matching, so `log` matches `log`, `logsize`, `logging` (token starts) but not `catalog` (mid-word) — unlike `Cmd+F`'s substring match. Both halves are accent-insensitive, so `cafe` finds Café: block search folds through FTS5, page names through the same locale-independent folding. The dialog is a fixed size (it does not resize as results change).
...
- **Tabs**: a window holds native macOS tabs (`Cmd+T` for a new one), each its own view with its own current page, history, and right sidebar — and its own graph, so one window can hold tabs on several graphs. `Cmd+1`…`Cmd+8` select that tab and `Cmd+9` the last one, whichever it is; macOS supplies the rest (`Ctrl+Tab` to step through them, Window ▸ Move Tab to New Window to detach one).
...
- **Copy** of a selected block subtree puts Markdown (with indentation) on the clipboard; **cut** writes the same Markdown and removes the blocks as one undo step (and writes nothing if the reference prompt in §7.4 is declined); **paste** of multi-line Markdown splits into blocks by list structure, or by lines if no list markers present. Exception: pasting into a quote, fenced-code, or table block — or pasting table-shaped text — keeps the lines inside that block (each is a single multi-line block by design).
...
- **Selection**: `Esc` from text editing selects the block (node selection); arrows extend selection across siblings; `Tab`/indent, move, delete, cut/copy/paste and Select All then operate on the whole selection — from the Edit menu as well as the keyboard.
