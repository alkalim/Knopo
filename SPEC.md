...
- **`Cmd+J`** opens the journal home; pressing it again, with the feed already showing, opens **today's own page** — a second press means "take me to today to write", and the caret lands there ready to type.
...
- Fast local query analysis strips conversational note-search framing, preserves the subject and requested evidence intent, and produces one focused topic plus high-signal lexical terms. The raw conversational question is never passed to FTS or the embedding model. A question with no searchable topic fails explicitly instead of searching question words.
- Ask runs exact-phrase, focused all-term, and any-focused-term lexical channels plus exactly one semantic query. Semantic proximity supplies terminology variants (for example, `anomaly detection` can retrieve `outlier detection`) without a separate generative planning call. Weighted reciprocal-rank fusion combines the channels. Ask removes duplicate text and prefers evidence from more than one page. Up to six retrieved anchor blocks are kept; direct parents and children may expand the final evidence set to eight, each with its own block id. Discovery requests such as “show/find notes about…” display those anchors directly and do not invoke the language model.
- For synthesis questions, one fresh on-device `LanguageModelSession` receives only the original question and evidence excerpts—focused search text and page names are not generation input. Each excerpt is split into locally numbered sentences. Foundation Models structured generation returns concise claims and source/sentence numbers; it does not copy quotations. Knopo validates each coordinate against the fixed prompt and supplies the exact stored sentence as the displayed supporting text. Fenced code, property-only blocks, and evidence whose detected language the system model does not support are excluded. The entire user prompt has a conservative 2,600-byte ceiling (not a per-block allowance), and response tokens are capped. A context-window rejection automatically retries once in a fresh session with a 1,400-byte prompt and smaller response allowance; a language rejection retries with evidence whose individual sentences match the question language. Invalid references and unsupported claims are discarded. Accepted citations navigate through `knopo://block/<id>`; if no claim survives, Knopo presents no generated answer.
