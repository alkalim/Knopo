...
# The icon is authored in Knopo.icon (Icon Composer) and compiled by actool to
# BOTH:
#   • Assets.car   → macOS 26 "Icon & widget style" theming (Default/Dark/Tinted/Clear)
#   • Knopo.icns   → the static icon macOS 15 and earlier use
# actool needs Xcode; swift build, test.sh and l10n-extract.sh do not.
if ! xcrun --find actool >/dev/null 2>&1; then
  echo "error: actool not found. Building Knopo.app needs Xcode installed." >&2
  exit 1
fi
xcrun actool Knopo.icon \
  --compile "$APP/Contents/Resources" \
  --app-icon Knopo \
  --platform macosx \
  --minimum-deployment-target 14.0 \
  --output-partial-info-plist "$(mktemp)" >/dev/null
if [ ! -f "$APP/Contents/Resources/Assets.car" ] || [ ! -f "$APP/Contents/Resources/Knopo.icns" ]; then
  echo "error: actool produced no Assets.car / Knopo.icns from Knopo.icon." >&2
  exit 1
fi
