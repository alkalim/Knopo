...
public final class GraphStore {
    ...
    public var dotDir: URL { root.appendingPathComponent(".everseq", isDirectory: true) }
    ...
    public init(root: URL) throws {
        ...
        for dir in [root.appendingPathComponent("pages"), root.appendingPathComponent("journals"),
                    root.appendingPathComponent(".everseq")] {
            try fm.createDirectory(at: dir, withIntermediateDirectories: true)
        }
        self.cache = try CacheDB(url: root.appendingPathComponent(".everseq/cache.db"))
        self.config = GraphConfig.load(from: root.appendingPathComponent(".everseq/config.json"))
    ...
    /// the losing in-memory version is saved to `.everseq/conflicts/`.
