...
final class GraphManager: ObservableObject {
    ...
    /// `GraphStore` lays out pages/, journals/ and .knopo/ on first open.
    ...
    static func defaultRoot() -> URL {
        if let path = ProcessInfo.processInfo.environment["KNOPO_GRAPH"], !path.isEmpty {
...
private struct WindowConfigurator: NSViewRepresentable {
    ...
    private static let windowsChanged = Notification.Name("knopoWindowsChanged")
